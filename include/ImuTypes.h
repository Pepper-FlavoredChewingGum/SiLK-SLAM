...
const float GRAVITY_VALUE=9.8066;//哈尔滨重力加速度

//IMU measurement (gyro, accelerometer and timestamp)
class Point//Point表示一个IMU的观测
{
public:
    Point(const float &acc_x, const float &acc_y, const float &acc_z,//加速度
             const float &ang_vel_x, const float &ang_vel_y, const float &ang_vel_z,//角速度
             const double &timestamp): a(acc_x,acc_y,acc_z), w(ang_vel_x,ang_vel_y,ang_vel_z), t(timestamp){}//时间戳
...
public:
    float bax, bay, baz;//加速度偏置
    float bwx, bwy, bwz;//角速度偏置
    EIGEN_MAKE_ALIGNED_OPERATOR_NEW
};
...
class IntegratedRotation//IMU角度预积分
{
public:
    IntegratedRotation(){}
...
public:
    ...
    Eigen::Matrix3f deltaR;//一个IMU数据的角度积分
    Eigen::Matrix3f rightJ; // right jacobian右雅可比矩阵
    EIGEN_MAKE_ALIGNED_OPERATOR_NEW
};

//Preintegration of Imu Measurements
class Preintegrated//最重要
{
    friend class boost::serialization::access;
    template<class Archive>
    void serialize(Archive & ar, const unsigned int version)
...
public:
    ...
    Eigen::DiagonalMatrix<float,6> Nga, NgaWalk;//噪声和随机游走

    // Values for the original bias (when integration was computed)
    Bias b;//优化前的偏置
    Eigen::Matrix3f dR;//角度预积分
...
private:
    ...
    Eigen::Matrix<float,6,1> db;//偏置更新的量
...
Eigen::Matrix3f RightJacobianSO3(const float &x, const float &y, const float &z);//右雅可比
Eigen::Matrix3f RightJacobianSO3(const Eigen::Vector3f &v);

Eigen::Matrix3f InverseRightJacobianSO3(const float &x, const float &y, const float &z);//右雅可比的逆
Eigen::Matrix3f InverseRightJacobianSO3(const Eigen::Vector3f &v);

Eigen::Matrix3f NormalizeRotation(const Eigen::Matrix3f &R);//对于输入的3x3旋转矩阵，强制归一化
