...
// 对关键帧相关数据进行保存
void Atlas::Save(const string &filename,const cv::MatSize image_size)
{
    ...
    // 输出关键帧的数量
    cout << "The number of KeyFrames: " << mpCurrentMap->GetAllKeyFrames().size() << endl;

    unsigned long int nKeyFrames = mpCurrentMap->GetAllKeyFrames().size();
    f << nKeyFrames << endl;
    std::vector<KeyFrame *> kfs = mpCurrentMap->GetAllKeyFrames();
    for(auto kf: kfs)
        SaveKeyFrame(f,kf);

    // 输出空间三维点的数目
    cout << "The number of MapPoints: " << mpCurrentMap->GetAllMapPoints().size() << endl;
    unsigned long int nMapPoints = mpCurrentMap->GetAllMapPoints().size();
    f << nMapPoints << endl;

    std::vector<MapPoint *> mps = mpCurrentMap->GetAllMapPoints();
    for(auto mp:mps)
        SaveMapPoint(f,mp);
...
//  保存地图点
void Atlas::SaveMapPoint(ofstream &f, MapPoint *mp)
{
    //保存当前MapPoint世界坐标值
...
//  保存关键帧
void Atlas::SaveKeyFrame(ofstream &f, KeyFrame *kf)
{
...
//     cout << "GetPose " << std::to_string(kf->mTimeStamp) <<"\nTcw\n" <<Tcw<< endl;
    ...
    cv::eigen2cv(Rcw, rotation);
//     cout << "Rcw\n" << Rcw << endl;
    // 通过四元数保存旋转矩阵
    std::vector<float> Quat = Converter::toQuaternion(rotation);
    
    Eigen::Vector3f translation_vector = Tcw.translation();
