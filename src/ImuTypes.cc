 ...
 */
IntegratedRotation::IntegratedRotation(const Eigen::Vector3f &angVel, const Bias &imuBias, const float &time)//关于角度的预积分，作用是计算角度预积分和右雅可比
{
    // 得到考虑偏置后的角度旋转
    ...
    if (d < eps)
    {
        ...
        rightJ = Eigen::Matrix3f::Identity();//右雅可比
    }
    else
    {
 ...
 */
Preintegrated::Preintegrated(const Bias &b_, const Calib &calib)//预积分类
{
    Nga = calib.Cov;
 ...
 * @param b_ 偏置
 */
void Preintegrated::Initialize(const Bias &b_)//初始化预积分，给预积分量赋初值
{
    dR.setIdentity();
    dV.setZero();
    dP.setZero();
 ...
 */
void Preintegrated::IntegrateNewMeasurement(const Eigen::Vector3f &acceleration, const Eigen::Vector3f &angVel, const float &dt)//输入是加速度、角速度和时间差，最重要
{
    ...
    acc << acceleration(0) - b.bax, acceleration(1) - b.bay, acceleration(2) - b.baz;//根据原始IMU数据减去偏执
    ...
    dP = dP + dV * dt + 0.5f * dR * acc * dt * dt;//偏置更新后，位置和速度的预积分更新
    dV = dV + dR * acc * dt;
    ...
    A.block<3, 3>(3, 0) = -dR * dt * Wacc;//噪声递推后，协方差矩阵中的A、B
 ...
 * @param pPrev 前面的预积分
 */
void Preintegrated::MergePrevious(Preintegrated *pPrev)//LoopMapping线程中，删除关键帧后，需要将两个关键帧之间的IMU数据合并
{
    if (pPrev == this)
        return;
 ...
 * @param naw 随机游走
 */
void Calib::Set(const Sophus::SE3<float> &sophTbc, const float &ng, const float &na, const float &ngw, const float &naw)//标定，把相关变量存到一个类中方便管理
{
    mbIsSet = true;
