   ...
   int ORBmatcher::SearchByBoW(KeyFrame* pKF,Frame &F, vector<MapPoint*> &vpMapPointMatches)
    {
        ...
        std::vector<int> vnMatches21(F.mvKeys.size(), -1);
        std::vector<float> distance(F.mvKeys.size(), -1);
                ...
                for(size_t iKF=0; iKF<vIndicesKF.size(); iKF++)
                {
                    ...
                    if(bestDist1<=TH_LOW)
                    {
                        ...
                        if(static_cast<float>(bestDist1)<mfNNratio*static_cast<float>(bestDist2))
                        {
                        	...
                        	vnMatches21[bestIdxF] = iKF;
                        	distance[bestIdxF] = bestDist1;
                        ...
                        if(bestDist1R<=TH_LOW)
                        {
                            ...
                            vnMatches21[bestIdxFR] = iKF;
                            distance[bestIdxFR] = bestDist1R;
        ...
        int match_ID = 0;
        std::vector<cv::KeyPoint> left_mvKeys = pKF->mvKeys, right_mvKeys = F.mvKeys;
        for(size_t i=0; i<vnMatches21.size(); i++){
                if(vnMatches21[i]==-1) {
//                     left_mvKeys[vnMatches21[i]].pt.x=0;
//                     left_mvKeys[vnMatches21[i]].pt.y=0;
//                     right_mvKeys[i].pt.x=0;
//                     right_mvKeys[i].pt.y=0;
                    continue;
                }
                matches.push_back(cv::DMatch(vnMatches21[i], i, distance[i]));
                match_ID++;
        }
         //cv::Mat matchImg;
         //cv::drawMatches(pKF->img_silk, left_mvKeys, F.img_silk, right_mvKeys, matches, matchImg, cv::Scalar::all(-1), cv::Scalar::all(-1), std::vector<char>(), cv::DrawMatchesFlags::NOT_DRAW_SINGLE_POINTS);
         //cv::imshow("silk_"+std::to_string(F.mnId)+"_"+std::to_string(match_ID), matchImg);
         //cv::imwrite("main/silk_"+std::to_string(F.mnId)+"_"+std::to_string(match_ID)+".jpg", matchImg);
         //cv::waitKey(0);
        return nmatches;
    }
    ...
    */
    ...
    int ORBmatcher::SearchByProjection(Frame &CurrentFrame, const Frame &LastFrame, const float th, const bool bMono)
    {
        ...
        std::vector<int> vnMatches21(CurrentFrame.mvKeys.size(), -1);
        std::vector<float> distance(CurrentFrame.mvKeys.size(), -1);
                ...
                if(!LastFrame.mvbOutlier[i])
                {
                    ...
                    if(CurrentFrame.Nleft != -1){
                        ...
                        if(bestDist<=TH_HIGH)
                        {
                            ...
                            vnMatches21[bestIdx2 + CurrentFrame.Nleft] = i;
                            distance[bestIdx2 + CurrentFrame.Nleft] = bestDist;
        ...
        int match_ID = 0;
        std::vector<cv::KeyPoint> left_mvKeys = LastFrame.mvKeys, right_mvKeys = CurrentFrame.mvKeys;
        for(size_t i=0; i<vnMatches21.size(); i++){
                if(vnMatches21[i]==-1) {
//                     left_mvKeys[vnMatches21[i]].pt.x=0;
//                     left_mvKeys[vnMatches21[i]].pt.y=0;
...
//                     right_mvKeys[i].pt.y=0;
         ...
         //cv::Mat matchImg;
         //cv::drawMatches(LastFrame.img_silk, left_mvKeys, CurrentFrame.img_silk, right_mvKeys, matches, matchImg, cv::Scalar::all(-1), cv::Scalar::all(-1), std::vector<char>(), cv::DrawMatchesFlags::NOT_DRAW_SINGLE_POINTS);
         //cv::imshow("silk_"+std::to_string(CurrentFrame.mnId)+"_"+std::to_string(match_ID), matchImg);
         //cv::imwrite("main/silk_"+std::to_string(CurrentFrame.mnId)+"_"+std::to_string(match_ID)+".jpg", matchImg);
         //cv::waitKey(0);
        return nmatches;
    }
