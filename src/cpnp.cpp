...
bool CPnP(std::vector<cv::Point2f>& point_2d,
    ...
    for(int i=0; i<point_2d.size(); i++){
        points_2d.push_back({point_2d[i].x, point_2d[i].y});
        // std::cout << "二维点的x坐标： " <<  points_2d[i](0) << "二维点的y坐标： " <<  points_2d[i](1) <<endl;
    }
    for(int i=0; i<point_2d.size(); i++){
        Eigen::Vector3d p = point_3d[i];
        points_3d.push_back(p);
        // std::cout << "三维点的x坐标： " <<  points_3d[i](0) << "三维点的y坐标： " <<  points_3d[i](1) << "三维点的z坐标： " <<  points_3d[i](2)<<endl;
    }
